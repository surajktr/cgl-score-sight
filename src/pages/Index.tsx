...
function parseQuestionsForPart(
  ...
  const getLanguageUrls = (imageUrl: string): { hindi: string; english: string } => {
    if (!imageUrl) return { hindi: '', english: '' };
    ...
    if (isHindi) {
      // Current URL is Hindi, keep it and generate English URL
      hindiUrl = imageUrl;
      ...
      // Current URL is English, keep it and generate Hindi URL
      englishUrl = imageUrl;
      ...
      if (extensionMatch) {
        ...
        if (['jpg', 'jpeg', 'png', 'gif'].includes(ext)) {
          ...
          // If extension is not recognized, use the original URL for both
          hindiUrl = imageUrl;
          englishUrl = imageUrl;
        }
      } else {
        // No extension found, use original URL for both as fallback
        hindiUrl = imageUrl;
        englishUrl = imageUrl;
      }
    }
  ...
  while ((tableMatch = questionTablePattern.exec(html)) !== null) {
    ...
    while ((optionMatch = optionRowPattern.exec(tableContent)) !== null && optIdx < 4) {
      ...
      // Debug logging for option URLs
      console.log(`Part ${part}, Q${qNum}, Option ${optionIds[optIdx]}:`, {
        original: optionImageUrl,
        hindi: optionLangUrls.hindi,
        english: optionLangUrls.english
      });
